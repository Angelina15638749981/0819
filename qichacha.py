...
if str(response.status_code) == "200":
    print("----------------------第1次requests请求获取--省市信息--成功-------------------------------")
else:
    print("----------------------第1次requests请求--省市信息--失败-------------------------------")
# print(response.text)
...
for i in range(1, 2):
    # 获取地址链接
    addr_link = html_area.xpath('//*[@id="area"]//div[1]/div[%s]/a/@href' % i)[0]
    # https://www.qichacha.com/g_BJ
    # full_addr_link = "https://www.qichacha.com" + addr_link
    # print(addr_link)
    # print(len(addr_link))
    # /g_BJ
    if len(addr_link) == 5:
    ...
    for x in range(1, 501):
        full_page_link = "https://www.qichacha.com/gongsi_area.html?prov="+addr_+"&p="+str(x)
        print("----------%s地区-------------第%s页---------------公司信息--------" % (addr_, x))
        print(full_page_link)
        # 获取公司链接
        # 进入每一页，获取公司链接和公司详细信息
        response_by_page_link = requests.get(full_page_link, headers=request_header_, )
        # print(response_by_page_link.text)
        # if str(response_by_page_link.status_code) == "200":
        #     print("----------------------第2次requests请求成功------------%s省市的企业信息-------------------" % addr_)
        # else:
        #     print("----------------------第2次requests请求失败-------------%s省市的企业信息------------------" % addr_)
...
#     full_link = "https://www.qichacha.com" + addr_link
#     print(full_link)
